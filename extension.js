...
function editHtml(docUri) {
	...
	return vscode.window.withProgress({
		...
		if (vscode.workspace.getConfiguration('md2singlehtml')['generateToc']) {
			...
			$('body').html('<div id="area__content">' + $('body').html() + '</div>');
			// メニューを作る
			$('body').append('<div id="area__menu"><div><p>Index</p><ul></ul></div></div>');
			const $ul = $('#area__menu ul');
			$('h1, h2').each(function () {
				const classnm = $(this).get(0).tagName.toLowerCase() === 'h1' ? 'menu-h1' : 'menu-h2';
				$ul.append('<li><a href="#' + $(this).attr('id') + '" class="' + classnm + '">' + $(this).text() + "</a></li>");
			});
			// CSS挿入
			$('head').append('<style>' + getDefaultMenuCss() + '</style>');
		}
		// ------------------
